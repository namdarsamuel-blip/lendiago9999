  ...
  <div class="relative container mx-auto px-4 md:px-6 pt-10 md:pt-14 pb-20 md:pb-32">
    ...
    <div class="max-w-3xl text-primary-foreground reveal-l">
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
        <a href="https://form.jotform.com/261271610738051" class="inline-flex items-center justify-center bg-accent text-primary hover:bg-white rounded-full px-9 py-4 text-base font-bold transition-all">
          Pre-qualify
...
<section class="bg-background border-b border-border/60">
  ...
  function productMain(p){
    const useCases = (p.useCases || p.longFeatures).slice(0,6);
    return `
<!-- USE CASES: image left + clean numbered list right -->
<section class="bg-muted/30 border-y border-border/60 py-24 md:py-32">
...
<!-- INTRO: simple, clean editorial — description + 3 process steps -->
<section id="details" class="bg-muted/30 border-y border-border/60 py-24 md:py-32">
  <div class="container mx-auto px-4 md:px-6">
    <div class="max-w-4xl mb-20 md:mb-24 reveal">
      ...
      <p class="text-2xl md:text-3xl lg:text-[2rem] font-headline text-primary leading-[1.3] tracking-tight">
        ${p.longDescription}
      </p>
    </div>

    <div class="reveal">
      <p class="text-[11px] font-semibold uppercase tracking-[0.28em] text-muted-foreground mb-10 md:mb-12 flex items-center gap-4">
        <span class="block h-px w-10 bg-accent"></span>
        How it works
      </p>
      <div class="grid md:grid-cols-3 gap-10 md:gap-12">
        ${[
          ['Apply','60 seconds, no documents to start. Soft credit pull, no score impact.'],
          ['Review','A specialist underwrites your file and matches you with the lender most likely to approve you the fastest, on the best terms.'],
          ['Funded','Pick the offer that fits, sign the contracts, and your capital is wired as fast as same day.']
        ].map(([t,d],i)=>`
          <div class="reveal" data-delay="${i*80}">
            <div class="flex items-baseline gap-3 mb-5">
              <span class="text-2xl md:text-3xl font-headline font-bold text-accent leading-none">${String(i+1).padStart(2,'0')}</span>
              <span class="block h-px flex-1 bg-border"></span>
            </div>
            <h4 class="text-lg md:text-xl font-bold font-headline text-primary leading-tight tracking-tight mb-3">${t}</h4>
            <p class="text-[15px] text-muted-foreground leading-relaxed">${d}</p>
          </div>`).join('')}
      </div>
    </div>
  </div>
  ...
  <div class="container mx-auto px-4 md:px-6 relative">
    <div class="grid md:grid-cols-12 gap-12 items-center">
      ...
      <div class="md:col-span-5 flex flex-col gap-3 md:items-end">
        <a href="https://form.jotform.com/261271610738051" class="inline-flex items-center justify-center bg-accent text-primary hover:bg-accent/90 rounded-full px-10 py-5 text-lg font-bold w-full md:w-auto">
          Apply Now
    ...
    <div class="grid md:grid-cols-12 gap-10 md:gap-12 lg:gap-16 items-stretch">
      ...
      <div class="md:col-span-6 lg:col-span-7 flex flex-col justify-center reveal py-4 md:py-12 lg:py-16">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-5">
          <a href="https://form.jotform.com/261271610738051" class="inline-flex items-center justify-center bg-accent hover:bg-accent/90 text-primary font-bold px-8 py-4 text-base shadow-xl shadow-accent/20 transition-all hover:scale-[1.02] rounded-full">
            Pre-qualify
          </a>
      ...
      <div class="md:col-span-6 lg:col-span-5 relative reveal" data-delay="120">
        <div class="relative h-full min-h-[360px] md:min-h-[460px] rounded-[1.5rem] md:rounded-[2rem] overflow-hidden">
          <img src="${ind.image}" alt="${ind.title}" class="absolute inset-0 w-full h-full object-cover" style="object-position:${ind.imagePos||'center top'}" referrerpolicy="no-referrer"/>
        </div>
      </div>
    ...
    <div class="border-t border-border/70 pt-8 md:pt-10">
      <div class="grid md:grid-cols-12 gap-x-10 gap-y-8 items-center">
        ...
        <div class="md:col-span-2 reveal flex md:justify-end" data-delay="180">
          <a href="https://form.jotform.com/261271610738051" class="inline-flex items-center gap-2 text-primary font-bold border-b-2 border-accent pb-1 hover:gap-3 transition-all text-sm whitespace-nowrap">
            Pre-qualify
      ...
      <div class="md:col-span-5 reveal">
        <div class="relative">
          ...
          <div class="relative aspect-[4/5] rounded-[1.75rem] overflow-hidden border border-border/60">
            <img src="${(ind.image2 || ind.image).replace(/(\?|&)w=\d+/,'$1w=600')}" alt="${ind.title}" class="w-full h-full object-cover" referrerpolicy="no-referrer" loading="lazy" decoding="async"/>
          </div>
...
<section class="bg-muted/30 border-y border-border/60 py-20 md:py-28">
  ...
  function industryBody(ind){
    const bySlug = (s)=> D.products.find(x=>x.slug===s);
    const recProducts = (ind.products && ind.products.length
      ? ind.products.map(bySlug).filter(Boolean)
      : D.products.slice(0,4));
    const variant = ['cards','rows','split'][
      [...ind.slug].reduce((a,c)=>a+c.charCodeAt(0),0) % 3
    ];
    const lower = ind.title.toLowerCase();
    ...
    let body = '';
    if(variant === 'rows'){
      // Horizontal row list with large index and right-side arrow
      body = `<div class="rounded-2xl border border-border/60 bg-white overflow-hidden divide-y divide-border/60">
        ${recProducts.map((p,i)=>`
        <a href="../${p.slug}.html" class="reveal group grid grid-cols-12 items-center gap-4 md:gap-8 px-6 md:px-10 py-7 md:py-9 hover:bg-muted/40 transition-colors" data-delay="${i*60}">
          <span class="col-span-2 md:col-span-1 text-2xl md:text-3xl font-bold font-headline text-accent leading-none">${String(i+1).padStart(2,'0')}</span>
          <div class="col-span-8 md:col-span-3 flex items-center gap-4">
            <div class="text-primary">${productDuoIcon(p.icon)}</div>
          </div>
          <div class="col-span-12 md:col-span-7 mt-2 md:mt-0">
            <h3 class="font-bold font-headline text-lg md:text-xl text-primary leading-tight tracking-tight group-hover:text-accent transition-colors mb-1.5">${p.title}</h3>
            <p class="text-sm text-muted-foreground leading-relaxed max-w-2xl">${p.description||''}</p>
          </div>
          <span class="hidden md:flex col-span-1 justify-end text-primary group-hover:text-accent transition-colors">
            <svg class="h-5 w-5 transition-transform group-hover:translate-x-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
          </span>
        </a>`).join('')}
      </div>`;
    } else if(variant === 'split'){
      // First product as a hero card, remaining as compact tiles
      const [first, ...rest] = recProducts;
      body = `<div class="grid lg:grid-cols-5 gap-5 md:gap-6">
        ${first ? `<a href="../${first.slug}.html" class="reveal group lg:col-span-2 bg-primary text-primary-foreground rounded-2xl p-10 md:p-12 flex flex-col hover:bg-primary/95 transition-colors" data-delay="0">
          <div class="flex items-start justify-between mb-12">
            <div class="text-accent">${productDuoIcon(first.icon)}</div>
            <span class="text-[10px] font-semibold tracking-[0.28em] text-white/40">RECOMMENDED</span>
          </div>
          <h3 class="font-bold font-headline text-2xl md:text-3xl leading-tight tracking-tight mb-4">${first.title}</h3>
          <p class="text-sm md:text-base text-white/70 leading-relaxed mb-8 flex-1">${first.description||''}</p>
          <span class="inline-flex items-center gap-2 text-sm font-semibold text-accent mt-auto">
            Explore ${first.title}
            <svg class="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
          </span>
        </a>` : ''}
        <div class="lg:col-span-3 grid sm:grid-cols-2 gap-5 md:gap-6">
          ${rest.map((p,i)=>`
          <a href="../${p.slug}.html" class="reveal group bg-white border border-border/60 rounded-2xl p-7 flex flex-col hover:border-primary hover:shadow-premium transition-all" data-delay="${(i+1)*60}">
            <div class="text-primary mb-6">${productDuoIcon(p.icon)}</div>
            <h3 class="font-bold font-headline text-lg text-primary leading-tight tracking-tight mb-2 group-hover:text-accent transition-colors">${p.title}</h3>
            <p class="text-sm text-muted-foreground leading-relaxed flex-1">${p.description||''}</p>
            <span class="inline-flex items-center gap-2 text-sm font-semibold text-primary group-hover:text-accent transition-colors mt-5">
              Learn more
              <svg class="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
            </span>
          </a>`).join('')}
      ...
      // Default cards grid (existing look)
      const cols = recProducts.length >= 4 ? 'lg:grid-cols-4' : (recProducts.length === 3 ? 'lg:grid-cols-3' : 'lg:grid-cols-2');
      body = `<div class="grid sm:grid-cols-2 ${cols} gap-5 md:gap-6">
        ${recProducts.map((p,i)=>`
    ...
    return `
<!-- Recommended products: varies per industry -->
<section class="py-20 md:py-28 bg-background">
  ...
  const NEEDS_CREDIT_REPORT = ['credit-stacking','business-line-of-credit','personal-term-loans'];
  ...
  <div class="container mx-auto px-4 md:px-6">
    <div class="max-w-5xl mx-auto">
      ...
      <div id="credit-report-submit" class="grid md:grid-cols-2 gap-px bg-border/60 border border-border/60 rounded-2xl overflow-hidden scroll-mt-24">
        <a href="https://form.jotform.com/261271610738051" class="group bg-primary text-white p-8 md:p-10 flex flex-col gap-4 hover:bg-primary/95 transition-colors">
