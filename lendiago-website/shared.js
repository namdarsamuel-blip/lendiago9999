...
<style id="lendiago-mobile-overrides">
  ...
  @media (max-width: 1023px) {
    ...
    /* Always leave room for the bottom tab bar */
  ...
  /* Active tab — pill behind icon + accent color */
  .lendiago-tab.is-active { color: var(--primary, #0a2540) !important; opacity: 1; }
  .lendiago-tab.is-active .tab-pill { background-color: rgba(10, 37, 64, 0.08); }
  .lendiago-tab.is-active svg { color: inherit; }
  .lendiago-tab:active .tab-pill { background-color: rgba(10, 37, 64, 0.12); transform: scale(0.94); }
...
<div id="mobileMenu" class="fixed inset-0 z-[100] hidden">
  <div class="absolute inset-0 bg-primary text-primary-foreground overflow-y-auto">
    ...
    <div class="px-6 pt-7 pb-2 space-y-3">
      <a href="https://form.jotform.com/261271610738051" target="_blank" rel="noopener" class="flex items-center justify-center gap-2 w-full bg-accent text-primary text-center font-bold text-base py-4 rounded-full shadow-lg">
        Apply Now
...
<nav id="mobileTabBar" class="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-background/92 backdrop-blur-xl border-t border-border/40" aria-label="Primary mobile navigation">
  <div class="relative grid grid-cols-4 max-w-2xl mx-auto px-2 pt-2.5 pb-[max(0.625rem,env(safe-area-inset-bottom))]">
    <a href="index.html" data-tab="home" class="lendiago-tab flex flex-col items-center gap-1 py-1.5 text-foreground/55 transition-colors">
      <span class="tab-pill flex items-center justify-center h-9 w-14 rounded-full transition-colors">
        <svg class="h-[22px] w-[22px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2Z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
      </span>
      <span class="text-[10px] font-semibold tracking-wider">Home</span>
    </a>
    <a href="financing-solutions.html" data-tab="solutions" class="lendiago-tab flex flex-col items-center gap-1 py-1.5 text-foreground/55 transition-colors">
      <span class="tab-pill flex items-center justify-center h-9 w-14 rounded-full transition-colors">
        <svg class="h-[22px] w-[22px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="14" rx="2"/><path d="M2 10h20"/><path d="M6 16h2"/><path d="M12 16h2"/></svg>
      </span>
      <span class="text-[10px] font-semibold tracking-wider">Solutions</span>
    </a>
    <a href="https://form.jotform.com/261271610738051" target="_blank" rel="noopener" data-tab="apply" class="lendiago-tab flex flex-col items-center gap-1 py-1.5 text-foreground/55 transition-colors">
      <span class="tab-pill flex items-center justify-center h-9 w-14 rounded-full transition-colors">
        <svg class="h-[22px] w-[22px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
      </span>
      <span class="text-[10px] font-semibold tracking-wider">Apply</span>
    </a>
    <button type="button" id="mobileTabMore" data-tab="more" class="lendiago-tab flex flex-col items-center gap-1 py-1.5 text-foreground/55 transition-colors" aria-label="Open menu">
      <span class="tab-pill flex items-center justify-center h-9 w-14 rounded-full transition-colors">
        <svg class="h-[22px] w-[22px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="4" x2="20" y1="6" y2="6"/><line x1="4" x2="20" y1="12" y2="12"/><line x1="4" x2="20" y1="18" y2="18"/></svg>
      </span>
      <span class="text-[10px] font-semibold tracking-wider">Menu</span>
    </button>
  </div>
</nav>
  `;
...
<footer class="relative border-t border-border/40 bg-muted/30 overflow-hidden">
  ...
  // Mark active tab based on LENDIAGO_NAV
  (function(){
    const tabMap = { home:'home', solutions:'solutions', partners:'partners', apply:'apply' };
    const cur = (window.LENDIAGO_NAV && window.LENDIAGO_NAV.current) || '';
    document.querySelectorAll('.lendiago-tab').forEach(t=>{
      if(t.dataset.tab === tabMap[cur]){
        t.classList.add('is-active','text-primary');
        t.classList.remove('text-foreground/55');
      }
    });
  })();
