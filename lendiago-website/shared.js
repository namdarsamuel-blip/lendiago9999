...
<style id="lendiago-mobile-overrides">
  ...
  @media (max-width: 1023px) {
    ...
    /* Buttons: minimum 48px hit target with tap feedback */
    a.inline-flex, button, .btn { min-height: 48px; transition: transform 0.12s ease-out, opacity 0.12s ease-out; }
    a.inline-flex:active, button:active, .btn:active { transform: scale(0.97); opacity: 0.9; }
    /* Pill buttons get extra horizontal padding for that premium feel */
    a.rounded-full, button.rounded-full { padding-left: 1.75rem; padding-right: 1.75rem; }

    /* Cards: soft, premium shadows */
    .shadow-premium, .shadow-2xl, .shadow-xl { box-shadow: 0 1px 2px rgba(15,23,42,0.04), 0 4px 16px rgba(15,23,42,0.06) !important; }
    .rounded-2xl, .rounded-3xl, .rounded-\\[2rem\\], .rounded-\\[2\\.5rem\\], .rounded-\\[3rem\\], .rounded-\\[1\\.5rem\\], .rounded-\\[1\\.75rem\\] { border-radius: 1rem !important; }

    /* Eyebrow / accent rule — keep tight, simple */
    .text-\\[11px\\], .text-\\[10px\\] { font-size: 0.6875rem !important; letter-spacing: 0.16em !important; }
  }
  ...
  @media (hover: none) {
    *:hover { transition: none !important; }
    ...
    .group:hover .group-hover\\:-translate-y-0\\.5 { transform: none !important; }
    .hover\\:-translate-y-1:hover { transform: none !important; }
    /* Disable underlines triggered by hover */
