...
<style id="lendiago-mobile-overrides">
  ...
  @media (max-width: 1023px) {
    ...
    /* Always leave room for the sticky CTA */
...
<!-- Mobile sticky CTA bar (appears after hero) -->
<div id="mobileStickyCTA" class="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-background/95 backdrop-blur-md border-t border-border/40 px-4 py-3 transition-all duration-300" style="transform: translateY(100%); opacity: 0; pointer-events: none;">
  <div class="flex items-center gap-2.5 max-w-2xl mx-auto">
    <a href="tel:5168350757" aria-label="Call Lendiago" class="flex-shrink-0 inline-flex items-center justify-center h-12 w-12 rounded-full border border-border bg-background hover:bg-muted transition text-primary">
      <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92Z"/></svg>
    </a>
    <a href="https://form.jotform.com/261271610738051" target="_blank" rel="noopener" class="flex-1 inline-flex items-center justify-center gap-2 bg-primary text-primary-foreground font-bold py-3.5 rounded-full text-sm shadow-md hover:bg-primary/90 transition">
      Get Funded Now
      <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
    </a>
  </div>
...
<footer class="relative border-t border-border/40 bg-muted/30 overflow-hidden">
  ...
  if(sticky){
    const onScroll = ()=>{
      // Reveal after scrolling past ~60% of viewport (past the hero)
      const threshold = Math.min(window.innerHeight * 0.6, 500);
