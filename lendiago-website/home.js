...
(function(){
  ...
  if(iTrack){
    const industriesData = (window.LENDIAGO_DATA && window.LENDIAGO_DATA.industries) || industries;
    iTrack.innerHTML = industriesData.map(i=>{
      const img = i.image || i.img;
      const title = i.title;
      const slug = i.slug;
      return `
      <div class="carousel-item shrink-0">
        <div class="h-full">
          <a href="industries/${slug}.html" class="block group">
            <div class="overflow-hidden relative rounded-2xl shadow-premium border-0 transition-all duration-500 group-hover:shadow-2xl">
              <div class="p-0 aspect-[3/4] flex items-center justify-center bg-muted relative">
                <img src="${img}" alt="${title}" class="absolute inset-0 object-cover w-full h-full transition-transform duration-700 group-hover:scale-110" style="object-position:${i.imagePos||'center top'}" referrerpolicy="no-referrer" />
                ...
                <div class="absolute bottom-6 left-6 right-6">
                  <h3 class="text-xl font-bold text-white font-headline tracking-wide group-hover:text-accent transition-colors">${title}</h3>
  ...
  function setupCarousel(trackId){
    ...
    const SMOOTH = 'transform 420ms cubic-bezier(0.22, 0.61, 0.36, 1)';
    track.style.transition = SMOOTH;

    /* Clone items at both ends so dragging past either edge shows real cards */
    const CLONES = originalCount;
    for(let i = 0; i < CLONES; i++){
      const c = originals[i % originalCount].cloneNode(true);
      c.dataset.clone = 'end';
      track.appendChild(c);
    }
    for(let i = CLONES - 1; i >= 0; i--){
      const c = originals[((i % originalCount)+originalCount) % originalCount].cloneNode(true);
      c.dataset.clone = 'start';
      track.insertBefore(c, track.firstChild);
    }
    const items = track.querySelectorAll('.carousel-item');
    let perView = 3, index = CLONES;
    let currentX = 0;
    ...
    function setX(x, animate){
      currentX = x;
      track.style.transition = animate ? SMOOTH : 'none';
    ...
    function layout(){
      ...
      setX(-stride()*index, false);
    }
    function goTo(i, animate=true){
      index = i;
      setX(-stride()*index, animate);
    }
    /* After animated transitions, normalize silently if we've drifted into clones */
    function normalize(){
      if(index >= CLONES + originalCount){
        index -= originalCount;
        setX(-stride()*index, false);
        // force reflow so re-enabling transitions doesn't replay
    ...
    document.querySelectorAll(`.carousel-prev[data-target="${trackId}"]`).forEach(b => b.addEventListener('click', ()=> goTo(index - 1)));
    document.querySelectorAll(`.carousel-next[data-target="${trackId}"]`).forEach(b => b.addEventListener('click', ()=> goTo(index + 1)));

    /* Pointer / touch drag — uses rAF for smoothness, velocity-based snap */
    let startX = 0, startY = 0, isDragging = false, lockedAxis = null;
    let startTransform = 0, lastMoveX = 0, lastMoveT = 0, velocity = 0, dragMoved = false, isMouse = false;
    let rafId = null, pendingX = 0;
    ...
    function dragStart(x, y, mouse){
      ...
      if(computed !== 'none'){
        ...
        if(m){
          ...
          startTransform = parts.length === 6 ? parts[4] : (parts[12] || 0);
        } else startTransform = currentX;
      } else startTransform = currentX;
      track.style.transition = 'none';
      track.style.transform = `translate3d(${startTransform}px,0,0)`;
      currentX = startTransform;
      startX = x; startY = y; isDragging = true; lockedAxis = null;
      isMouse = !!mouse; dragMoved = false;
      lastMoveX = x; lastMoveT = performance.now(); velocity = 0;
      if(mouse) track.style.cursor = 'grabbing';
    }
    function dragMove(x, y){
      ...
      if(lockedAxis === 'x'){
        ...
        pendingX = startTransform + dx;
        scheduleFrame();
      }
    }
    function dragEnd(){
      if(!isDragging) return;
      const wasX = lockedAxis === 'x';
      isDragging = false;
      track.style.cursor = '';
      if(!wasX){ return; }
      const s = stride();
      const draggedItems = (currentX - startTransform) / -s;
      // Add velocity-based momentum (px/ms * factor) — heavier weight for fast flicks
      const momentum = -velocity * 220 / s;
      const targetItems = draggedItems + momentum;
      const itemsMoved = Math.round(targetItems);
      goTo(index + itemsMoved, true);
    }
    /* Touch swipe — touch-action: pan-y on the track lets browser handle vertical scroll natively */
    track.addEventListener('touchstart', (e)=>{ const t=e.touches[0]; dragStart(t.clientX, t.clientY, false); }, { passive: true });
    track.addEventListener('touchmove', (e)=>{ const t=e.touches[0]; dragMove(t.clientX, t.clientY); }, { passive: true });
    track.addEventListener('touchend', dragEnd);
    track.addEventListener('touchcancel', dragEnd);
    /* Mouse drag — desktop */
    track.style.cursor = 'grab';
    track.addEventListener('mousedown', (e)=>{ e.preventDefault(); dragStart(e.clientX, e.clientY, true); });
    window.addEventListener('mousemove', (e)=>{ if(isDragging && isMouse) dragMove(e.clientX, e.clientY); });
    window.addEventListener('mouseup', ()=>{ if(isDragging && isMouse) dragEnd(); });
    /* Prevent clicks from firing after a drag */
    track.addEventListener('click', (e)=>{ if(dragMoved){ e.preventDefault(); e.stopPropagation(); dragMoved = false; } }, true);
