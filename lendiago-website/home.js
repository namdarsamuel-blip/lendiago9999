...
(function(){
  ...
  const flexPills = ['Term Loans','Merchant Cash Advance','Business Line Of Credit','Credit Stacking','SBA Loans','Real Estate Loan','Personal Term Loans'];
  ...
  function setupCarousel(trackId){
    ...
    /* MOBILE: native CSS scroll-snap. Browser handles momentum + flick perfectly. */
    function setupMobile(){
      // Reset any transforms / clones / inline widths
      track.querySelectorAll('[data-clone]').forEach(c => c.remove());
      track.style.transform = '';
      track.style.transition = '';
      track.style.willChange = '';
      track.style.cursor = '';
      // Apply native scroll styles
      ...
      track.style.gap = '0.75rem';
      // Width: 80vw per item with snap-align start
      track.querySelectorAll('.carousel-item').forEach(it => {
        it.style.width = '80vw';
        it.style.maxWidth = '340px';
        it.style.scrollSnapAlign = 'start';
        it.style.flexShrink = '0';
      });
      // Add left/right padding so first/last cards align with the viewport edge
      track.style.paddingLeft = '1rem';
      track.style.paddingRight = '1rem';
      track.style.scrollPaddingLeft = '1rem';
      // Wire arrows to native scroll
      const stride = ()=>{
        const it = track.querySelector('.carousel-item');
        return it ? it.offsetWidth + 12 : 300;
      };
      document.querySelectorAll(`.carousel-prev[data-target="${trackId}"]`).forEach(b => {
        b.onclick = ()=> viewport.scrollBy({ left: -stride(), behavior: 'smooth' });
      });
      document.querySelectorAll(`.carousel-next[data-target="${trackId}"]`).forEach(b => {
        b.onclick = ()=> viewport.scrollBy({ left: stride(), behavior: 'smooth' });
      });
    }

    /* DESKTOP: JS-driven infinite loop with mouse drag */
    let originals = Array.from(track.querySelectorAll('.carousel-item'));
    const originalCount = originals.length;
    let perView = 3, index = 0, currentX = 0;
    let CLONES = 0;
    const SMOOTH = 'transform 420ms cubic-bezier(0.22, 0.61, 0.36, 1)';
    let resizeBound = false;

    function setupDesktop(){
      // Strip any native scroll styles
      ...
      // Remove old clones, re-clone
      track.querySelectorAll('[data-clone]').forEach(c => c.remove());
      originals = Array.from(track.querySelectorAll('.carousel-item'));
      if(originals.length === 0) return;
      CLONES = originals.length;
      for(let i = 0; i < CLONES; i++){
        const c = originals[i % originals.length].cloneNode(true);
        c.dataset.clone = 'end';
        track.appendChild(c);
      }
      for(let i = CLONES - 1; i >= 0; i--){
        const c = originals[((i % originals.length)+originals.length) % originals.length].cloneNode(true);
        c.dataset.clone = 'start';
        track.insertBefore(c, track.firstChild);
      }
      // Reset snap styles on items
      track.querySelectorAll('.carousel-item').forEach(it => {
        it.style.scrollSnapAlign = '';
      });
      index = CLONES;
      calcPerView();
      bindControls();
    }
    function items(){ return track.querySelectorAll('.carousel-item'); }
    function calcPerView(){
      const w = window.innerWidth;
      if(trackId === 'industriesTrack') perView = w >= 1024 ? 4 : 3;
      else perView = w >= 1024 ? 3 : 2;
      layout();
    }
    function stride(){
      const gap = 16;
    ...
    function bindControls(){
      track.addEventListener('transitionend', (e)=>{
        if(e.propertyName === 'transform' && !isMobile()) normalize();
      });
      document.querySelectorAll(`.carousel-prev[data-target="${trackId}"]`).forEach(b => {
        b.onclick = ()=> { if(!isMobile()) goTo(index - 1); };
      });
      document.querySelectorAll(`.carousel-next[data-target="${trackId}"]`).forEach(b => {
        b.onclick = ()=> { if(!isMobile()) goTo(index + 1); };
      });
      ...
      let startX = 0, isDragging = false, startTransform = 0, dragMoved = false, velocity = 0, lastMoveX = 0, lastMoveT = 0;
      track.addEventListener('mousedown', (e)=>{
        ...
        startX = e.clientX; isDragging = true; dragMoved = false;
      ...
      window.addEventListener('mouseup', ()=>{
        ...
        const itemsMoved = Math.round(draggedItems + momentum);
    ...
    /* Init based on viewport size, re-init on resize across breakpoint */
